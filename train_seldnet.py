...
def main(argv):
    ...
    if '2020' in params['dataset_dir']:
        ...
        test_splits = [[5]] # evaluate on dev_test
        val_splits = [[4]] # validate on dev_train
        train_splits = [[1, 2, 3, 6, 7, 8]] # only train on synthetic data
    ...
    if params['mode'] == 'dev':
        for split_cnt, split in enumerate(test_splits):
            ...
            for epoch_cnt in range(nb_epoch):
                ...
                # Print stats
                print('epoch: {}, time: {:0.2f}/{:0.2f}, '
                      'train_loss: {:0.4f}, val_loss: {:0.4f}, '
                      'ER/F/LE/LR/SELD: {}, '
                      'best_val_epoch: {} {}'.format(epoch_cnt, train_time, val_time, train_loss, val_loss,
                                                     '{:0.2f}/{:0.2f}/{:0.2f}/{:0.2f}/{:0.2f}'.format(val_ER, val_F, val_LE,
                                                                                                      val_LR, val_seld_scr),
                                                     best_val_epoch,
                                                     '({:0.2f}/{:0.2f}/{:0.2f}/{:0.2f}/{:0.2f})'.format(best_ER, best_F,
                                                                                                        best_LE, best_LR,
                                                                                                        best_seld_scr))
                      )
        ...
        for split_cnt, split in enumerate(test_splits):
            ...
            cls_feature_class.create_folder(params['model_dir'])
            model_name = params['pretrained_model_weights']
